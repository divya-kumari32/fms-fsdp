...
from fms_fsdp.models.block_ra_llama import BlockRALLaMA, BlockRATransformerBlock
...
def main(**kwargs):
    ...
    if cfg.low_cpu_fsdp:
        with torch.device("meta"):
            model = BlockRALLaMA(llama_config, cp_mesh=cp_mesh, num_slots=cfg.ra_num_slots, ac_group_size=cfg.ra_ac_group_size)
    else:
        model = BlockRALLaMA(llama_config, cp_mesh=cp_mesh, num_slots=cfg.ra_num_slots, ac_group_size=cfg.ra_ac_group_size)
    ...
    if rank == 0:
        ...
        print(f"\n--> BlockRALLaMA (faithful Block Attention Residuals) has {total_params / 1e6} Million params, N={cfg.ra_num_slots} blocks x S={llama_config.nlayers // cfg.ra_num_slots} layers/block\n")
    ...
    model.rot_emb.compute_freqs_cis(
        ...
        llama_config.max_expected_seq_len,
    )

    if cfg.fsdp_activation_checkpointing:
        if cfg.ra_ac_group_size > 0:
            # The model self-checkpoints in layer groups inside BlockRALLaMA.forward
